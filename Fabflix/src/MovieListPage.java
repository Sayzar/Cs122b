...
public class MovieListPage extends HttpServlet {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
		...
		try {
			...
			out.println("<HTML><HEAD><TITLE>FabFlix - MovieList</TITLE>" + "<script src='/Fabflix/js/lib/jquery.js' type='text/javascript'></script>"
					+ "<script src='/Fabflix/js/movie-details.js' type='text/javascript'></script>"
					+ "<script src='/Fabflix/sorttable.js' type='text/javascript'></script>"
					+ "<script src='/Fabflix/paging.js' type='text/javascript'></script>" + "<link rel='StyleSheet' href='../pagingStyle.css'>" + "</HEAD>");
	...
	private void createResultsTable(Connection dbcon, ArrayList<Movie> results, PrintWriter out) throws SQLException {
		out.println("<TABLE class='sortable' id='results' border>");
		out.println("<tr>" + "<th>id</th>" + "<th>title</th>" + "<th>year</th>" + "<th>director</th>" + "<th>genres</th>" + "<th>stars</th>");
		...
		for (int i = 0; i < results.size(); i++) {
			...
			if (!m_BU.equals("n/a") || !m_BU.equals("")) {
				m_TL = "<a href='/Fabflix/MovieInfo?movieid=" + m_ID + "' class='movie-title' data-mid ='" + m_ID + "'>" + m_TL + "</a>";
			}

			out.println("<tr>" + "<td>" + m_ID + "</td>" + "<td>" + m_TL + "</td>" + "<td>" + m_YR + "</td>" + "<td>" + m_DR + "</td>" + "<td>" + m_GR
					+ "</td>" + "<td>" + m_ST + "</td>" + "</tr>");
		}
	...
	private String getGenresInMovieString(Connection dbcon, String m_ID) throws SQLException {
		String query = "SELECT name FROM (genres g JOIN genres_in_movies gm " + "ON g.id = gm.genre_id) WHERE gm.movie_id=" + m_ID;
	...
	private String getStarsInMovieString(Connection dbcon, String m_ID) throws SQLException {
